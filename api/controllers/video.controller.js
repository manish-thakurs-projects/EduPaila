...
function validateUserPermissions(video, userId, res, next) {
  ...
  if (video.userId !== userId) return next(errorHandler(403, 'You are not allowed to perform this action'));
}

export const createVideo = async (req, res, next) => {
  try {
    const { title, description, youtubeUrl, category } = req.body;
    if (!title || !youtubeUrl || !category) return next(errorHandler(400, 'Title, YouTube URL, and Category are required'));
    
    const videoId = extractVideoIdFromUrl(youtubeUrl);
    if (!videoId) return next(errorHandler(400, 'Invalid YouTube URL'));
    
    const thumbnail = `https://img.youtube.com/vi/${videoId}/hqdefault.jpg`;
    const newVideo = new Video({ title, description, youtubeUrl, thumbnail, category, userId: req.user.id });
...
export const getVideos = async (req, res, next) => {
  try {
    ...
    const videos = await Video.find().sort({ createdAt: -1 }).skip(startIndex).limit(limit);
...
export const getVideoDetails = async (req, res, next) => {
  try {
    ...
    const comments = await Comment.find({ videoId: req.params.videoId }).sort({ createdAt: -1 }).skip(skip).limit(limit).populate('userId', 'username');
...
export const likeVideo = async (req, res, next) => {
  try {
    ...
    userIndex === -1 ? video.likes.push(req.user.id) : video.likes.splice(userIndex, 1);
