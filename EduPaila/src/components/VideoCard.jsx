...
const VideoCard = ({ video }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        ...
        !reportPopupRef.current.contains(event.target)
      ) {
  ...
  const handleSendReport = () => {
    const message = encodeURIComponent(
      `Report for video: ${videoData.title}\n\nIssue: ${reportMessage}`
    );
    ...
    >
          ...
          <div className="flex justify-between">
            ...
            <div className="relative flex items-center justify-center">
              ...
              {dropdownVisible && (
                <div
                  ref={dropdownRef}
                  className="absolute right-0 mt-2 w-40 bg-white border rounded-lg shadow-lg text-gray-700"
                >
                  <div
                    onClick={handleReport}
                    className="px-4 flex py-2 cursor-pointer hover:bg-gray-100 justify-center items-center"
                  >
      ...
      {isReportOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50">
          <div
            ref={reportPopupRef}
            className="bg-slate-200 dark:bg-slate-900 rounded-lg p-6 max-w-sm w-full"
          >
            <h3 className="text-lg font-semibold">Report Issue</h3>
            <p className="text-red-400 p-5">
              Please be respectful when reporting issues. Abusive language will
              not be tolerated. Kindly note that your report may be recorded for
              quality and security purposes. Thank you for your cooperation.
            </p>
            <textarea
              value={reportMessage}
            ...
            <div className="flex justify-between">
              ...
              <Button onClick={handleSendReport} outline color="green" pill>
                Send
              </Button>
            </div>
          </div>
